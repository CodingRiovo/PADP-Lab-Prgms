...
void mark_composites(bool *composite, long start, long step, long limit)
{
...
{
    long count = 0;
    bool *composite = calloc(n + 1, sizeof(bool));
    long limit = sqrt(n);
...
long sieve_friendly(long n)
{
    ...
    bool *small_comp = calloc(limit + 1, sizeof(bool));
    ...
    for (long start = limit + 1; start <= n; start += limit)
    {
        ...
        bool *segment = calloc(limit, sizeof(bool));
...
#pragma omp parallel
    {
        bool *local_comp = calloc(limit + 1, sizeof(bool));
        long *marker = malloc(n_factor * sizeof(long));

#pragma omp for reduction(+ : count)
        for (long start = limit + 1; start <= n; start += limit)
        {
            memset(local_comp, 0, (limit + 1) * sizeof(bool));
            long end = (start + limit - 1 < n) ? start + limit - 1 : n;
